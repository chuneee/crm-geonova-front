...
export function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // El mensaje del backend viene en err.response.data.message
      if (err.response?.data?.message) {
        // Si message es un array, tomar el primer elemento
        const message = Array.isArray(err.response.data.message)
          ? err.response.data.message[0]
          : err.response.data.message;
        setError(message);
        // console.log("📝 Mensaje de error establecido:", message);
      ...
      <div className="w-full max-w-6xl grid lg:grid-cols-2 gap-8 items-center">
        ...
        <div className="hidden lg:flex flex-col justify-center p-12">
          ...
          <p className="text-lg text-[#64748B] mb-12">
            Administra leads, clientes, cotizaciones e instalaciones desde una
            sola plataforma moderna y eficiente.
          </p>
        ...
        <div className="w-full max-w-md mx-auto">
          <div className="bg-white rounded-2xl shadow-2xl border border-[#E2E8F0] p-8 lg:p-10">
            ...
            <div className="mb-8">
              <h2 className="text-2xl text-[#1E293B] mb-2">
                Bienvenido de nuevo
              </h2>
            ...
            {/* Error Message - Movido antes del formulario para mejor visibilidad */}
            {error && (
              <div
                className="mb-6 p-3 bg-red-50 border border-red-200 text-red-700 rounded-lg text-sm flex items-center gap-2"
                role="alert"
              >
                <svg
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label
                  htmlFor="email"
                  className="block text-sm text-[#1E293B] mb-2"
                >
                  Correo electrónico
                </label>
