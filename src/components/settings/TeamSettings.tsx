...
export function TeamSettings() {
  ...
  const initialData = async () => {
    try {
      const usersResponse = await UserAPI.findAllUsers();
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#64748B]">
            Gestiona los miembros y permisos de tu equipo
          </p>
        </div>
      ...
      <div className="bg-white rounded-lg border border-[#E2E8F0]">
        <div className="px-6 py-4 border-b border-[#E2E8F0]">
          <h3 className="text-sm text-[#1E293B]">
            Miembros Activos ({users.filter((u) => u.active == true).length})
          </h3>
        </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-gradient-to-br from-[#3B82F6] to-[#8B5CF6] rounded-full flex items-center justify-center text-white flex-shrink-0">
                        {member.names.charAt(0)} {member.surnames.charAt(0)}
                      </div>
